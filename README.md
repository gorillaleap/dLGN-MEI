...
本项目支持不同尺寸的圆形感受野遮罩（Circular Mask），用户可根据目标神经元的生物学感受野大小选择合适的配置。

#### 可用 Mask 选项

| Mask 名称 | 直径（像素） | 等效原图感受野 | 适用场景 |
|----------|------------|--------------|---------|
| **Default** | 100 px | 50 px | 标准配置，适用于大多数 RGC 神经元 |
| **small** | 60 px | 30 px | 较小感受野，针对稀疏刺激优化 |
| **small-55** | 55 px | 27.5 px | 极限中心聚焦，最大化信噪比 |

> **注意**：由于采用了 2x 空间尺度重映射（512×640 → 1024×1280），裁剪框尺寸除以 2 即为等效的原始图像感受野大小。

#### 如何切换 Mask 配置

额外的 `small` 和 `small-55` Mask 配置文件已打包在项目附带的压缩包中。根据您对目标神经元生物学感受野的预期，按以下步骤切换配置：
