...
class FastCUDADataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 取出的数据已经是 CUDA Tensor 了，没有 CPU 传输瓶颈
...
class PooledStacked2dCore(nn.Module):
    ...
    包装 Stacked2dCore，实现：
    1. 只输出最后一层特征
    2. 添加 AvgPool2d(2) 进行空间下采样 (36×64 → 18×32)
    """
...
# ==========================================
def run_training():
    swanlab.init(project="MEI-Final", experiment_name="4090-InVRAM-Lightning")
    device = torch.device('cuda')

    # 🌟 初始化时直接传入 device，数据全部驻留显存
    dataset = FastCUDADataset('my_training_data.mat', device)
    train_split = int(0.8 * len(dataset))
    
    # 🌟 去掉了所有拖慢 Windows 速度的参数 (num_workers, pin_memory)
    ...
    print(f"🔥 4090 就绪！可训练参数: {sum(p.numel() for p in trainable_params)}")
    ...
    torch.save(model.state_dict(), 'best_pro_model.pth')
    print("✅ 模型已保存为 best_pro_model.pth")
